...
### 2. Search over the library — the one unmet need, measured

**Bento documents are invisible to the operating system's search.** Verified with
a control: a plain HTML file with a phrase in `<body>` is found by Spotlight; the
same phrase inside a `<script>` block is not; a real deck, whose content lives in
`#bento-doc`, is not found — though `grep` finds it immediately.

So "find the deck where I showed Q3 churn" is genuinely unserved by the OS the
user already runs. This is the only part of the original vault promise that no
existing tool has quietly taken, and the same index serves both search and
retrieval for the broker.

**One cheaper avenue must be tested before building an indexer.** Text inside
`<noscript>` *is* indexed by Spotlight (measured), and since v1.0.11 every save
writes a page-one render into `<noscript data-bento-preview>`. If that block
carried the text of every slide, decks would become searchable with **no vault at
all**. Unverified end-to-end — no deck in the repo carries a preview block yet,
and generating one needs a real DOM. Constraints: `PREVIEW_BUDGET` is 64KB, and
anything written into a document is additive forever (`PLATFORM.md` §3).
...
1. **Make the self-hosting claim true.** The r/selfhosted copy already says the
   ...
   this audience.
2. **Test the `<noscript>` search avenue** before building any indexer. If it
   works, most of pillar 2 ships with no server.
